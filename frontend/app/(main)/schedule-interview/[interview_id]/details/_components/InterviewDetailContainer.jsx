...
const InterviewDetailContainer = ({ interviewDetails }) => {
    // `type` is stored as "Technical, Behavioural" (older rows may hold a JSON array string)
    let parsedType = [];
    const rawType = interviewDetails?.type;
    if (Array.isArray(rawType)) {
        parsedType = rawType;
    } else if (typeof rawType === 'string') {
        try {
            const parsed = JSON.parse(rawType);
            parsedType = Array.isArray(parsed) ? parsed : [String(parsed)];
        } catch {
            parsedType = rawType.split(',').map((item) => item.trim()).filter(Boolean);
        }
    }
